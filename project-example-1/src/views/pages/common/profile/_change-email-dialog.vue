    ...
    <div class="modal-card" style="max-width: 80%; margin: 0 auto;">
      <form @submit.prevent="submitForm" name="email-change" method="post">
        ...
        <header class="modal-card-head">
          <p class="modal-card-title">{{ $t('block-global.email-changing') }}</p>
        </header>
        ...
        <section class="modal-card-body">
          ...
          >
            <b-input
              ...
              autocomplete="new-email"
              :disabled="isFormProcessing"
              :placeholder="$t('block-global.new-email-confirmation-placeholder')"
            />
          </b-field>
...
import {
  required, minLength, maxLength, sameAs, email,
...
export default {
  ...
  computed: {
    ...
    emailConfirmErrorMessages() {
      ...
      if (!this.$v.confirmEmail.required) errors.push(this.$t('block-errors.field-is-required'));
      if (!this.$v.confirmEmail.sameAsPassword) errors.push(this.$t('block-errors.email-not-equal'));

      return errors;
    },

    formErrorsMessages() {
      return this.formErrors.map(item => this.$t(`block-errors.change-email.${item}`));
    },
  },
    ...
    submitForm() {
      ...
      if (!this.$v.$invalid) {
        this.changeEmail({
          ...
          .then((response) => {
            if (response) {
              return this.sendVerificationCodeToNewEmail(this.$_user.uid)
                .then(() => {
                  this.isOpened = false;
