...
async function validateRegistrationFields(firstName, email, phone, bonusHolderNick, locale) {
  const errors = [];
...
async function validateSignUpToWebinarFields(firstName, phone) {
  const errors = [];
...
async function validateEnrollToWebinarFields(name, email, phone, city) {
  const errors = [];
  ...
  if (name && email && phone && city) {
    ...
    const validEmailRegex = new RegExp(VALID_EMAIL_REGEX);
    const validPhoneRegex = new RegExp(VALID_PHONE_REGEX);
    const validCityRegex = new RegExp(VALID_NAME_REGEX);
    ...
    if (name.length < NAME_MIN_LENGTH) {
      errors.push('invalid-name-minimum-length');
    }

    if (name.length > NAME_MAX_LENGTH) {
      errors.push('invalid-name-maximum-length');
    }
    ...
    if (city.length < CITY_MIN_LENGTH) {
      errors.push('invalid-city-minimum-length');
    }

    if (city.length > CITY_MAX_LENGTH) {
      errors.push('invalid-city-maximum-length');
    }
  }
...
module.exports = {
  ...
  validateSignUpToWebinarFields,
  validateEnrollToWebinarFields,
